...
describe("Docs recursive listing and ID generation", () => {
	...
	it("lists and views documents from subdirectories and generates unique IDs", async () => {
		...
		assert.ok(docIds.includes("doc-1"));
		assert.ok(docIds.includes("doc-2"));
		assert.ok(docIds.includes("doc-3"));
		...
		assert.strictEqual(doc2?.title, "Nested A");
		...
		assert.strictEqual(hasDoc4, true);
