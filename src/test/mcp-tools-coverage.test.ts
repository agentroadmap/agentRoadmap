...
import { registerMessageTools } from "../mcp/tools/messages/index.ts";
import { registerTeamTools } from "../mcp/tools/teams/index.ts";
...
describe("MCP spending tools", () => {
	beforeEach(async () => {
		...
		const { registerSpendingTools } = await import("../mcp/tools/spending/index.ts");
...
describe("MCP memory tools", () => {
	beforeEach(async () => {
		...
		execSync(`git config user.email test@example.com`, { cwd: TEST_DIR });
		await mcpServer.initializeProject("Test Project");
		const { registerMemoryTools } = await import("../mcp/tools/memory/index.ts");
...
describe("MCP model tools", () => {
	beforeEach(async () => {
		...
		execSync(`git config user.email test@example.com`, { cwd: TEST_DIR });
		await mcpServer.initializeProject("Test Project");
		const { registerModelTools } = await import("../mcp/tools/models/index.ts");
...
describe("MCP agent pool — advanced operations", () => {
	beforeEach(async () => {
		...
		execSync(`git config user.email test@example.com`, { cwd: TEST_DIR });
		await mcpServer.initializeProject("Test Project");
		const { registerAgentTools } = await import("../mcp/tools/agents/index.ts");
