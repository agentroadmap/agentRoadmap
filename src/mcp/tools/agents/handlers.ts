 ...
 * Dynamic multi-model agent pool management via SpacetimeDB
 */
...
// MCP-specific types — base types come from spacetime/tables/agent-pool.ts
import type {
	AgentProfile,
	AgentProvider,
	AgentConfig,
	AgentWorkClaim,
} from "../../../spacetime/tables/agent-pool.ts";
...
export class AgentPoolHandlers {
	...
	// In-memory store (will be replaced with SpacetimeDB queries)
	...
	/**
	 * AC#5: Detect zombie agents (no heartbeat for 5+ minutes)
	 */
	async detectZombies(): Promise<CallToolResult> {
		try {
			...
			for (const [id, agent] of this.agents) {
				...
				if (age > threshold) {
					agent.status = "offline";
					agent.lastError = `Zombie detected: no heartbeat for ${Math.round(age / 60000)} minutes`;
					zombies.push(id);
...
const S100_DB = 'c200f764d605d57af9030c9193af0211fa0f01cbe719e9a6560490b95bd08b48';
