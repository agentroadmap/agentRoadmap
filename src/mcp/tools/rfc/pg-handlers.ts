...
async function resolveProposalRecord(identifier: string): Promise<ResolvedProposal | null> {
...
async function loadMissingRequiredFields(proposal: ResolvedProposal): Promise<string[]> {
...
async function hasOutstandingAcceptanceCriteria(proposalId: number): Promise<boolean> {
...
function transitionNeedsAcceptanceCriteria(definition: TransitionDefinition): boolean {
...
function deriveTransitionReason(definition: TransitionDefinition, fromState: string, toState: string): string {
  return definition.allowed_reasons?.[0]
    ?? definition.labels?.[0]
    ?? classifyTransition(fromState, toState);
}

function deriveMaturityLabel(proposal: ResolvedProposal, fromState: string, toState: string): string {
  return proposal.maturity?.[toState]
...
export async function transitionProposal(args: {
  ...
  try {
    ...
    if (transitionNeedsAcceptanceCriteria(transition) && await hasOutstandingAcceptanceCriteria(proposal.id)) {
      return {
        content: [{
          type: "text",
    ...
    const { rowCount } = await query(
      ...
      [reason, args.decided_by, args.rationale || null, proposal.id, fromState, toState],
    );
    ...
    if ((rowCount ?? 0) === 0) {
      await query(
        `INSERT INTO proposal_state_transitions
         ...
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [proposal.id, fromState, toState, reason, args.rationale || null, args.decided_by],
      );
    }
...
export async function verifyAC(args: {
  ...
  try {
    ...
    await query(
      ...
      [args.status, args.verified_by, args.verification_notes || null, proposalId, args.item_number]
    );
  ...
  try {
    ...
    if (existing.length) {
      await query(
        ...
        [args.verdict, args.notes || null, args.findings ? JSON.stringify(args.findings) : null, proposalId, args.reviewer]
      );
    } else {
      await query(
        `INSERT INTO proposal_reviews (proposal_id, reviewer_identity, verdict, notes, findings)
         VALUES ($1, $2, $3, $4, $5)`,
        [proposalId, args.reviewer, args.verdict, args.notes || null, args.findings ? JSON.stringify(args.findings) : null]
      );
    }
...
export async function addDiscussion(args: {
  ...
  try {
    ...
    const { rows } = await query(
      ...
      [proposalId, args.author, args.content, args.parent_id || null, args.context_prefix || 'general:']
    );
    ...
    this.server.addTool({
      ...
      inputSchema: {
        ...
        properties: {
          ...
          dep_type: { type: "string", enum: ["blocks", "depended_by", "supersedes", "relates"], default: "blocks" },
        },
