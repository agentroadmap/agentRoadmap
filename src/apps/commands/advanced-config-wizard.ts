	...
	const crossBranchPrompt = await promptImpl(
		{
			...
			message: "Check proposal proposals across active branches?",
			hint: "Ensures accurate proposal tracking across branches (may impact performance on large repos)",
		...
		const remotePrompt = await promptImpl(
			{
				...
				message: "Check proposal proposals in remote branches?",
				hint: "Required for accessing proposals from feature branches on remote repos",
		...
		const zeroPaddingPrompt = await promptImpl(
			{
				...
				hint: "Example: proposal-001, doc-001 instead of proposal-1, doc-1",
		...
		const paddingPrompt = await promptImpl(
			{
				...
				hint: "e.g., 3 creates proposal-001; 4 creates proposal-0001",
