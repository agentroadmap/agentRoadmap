...
export class PgAgentHandlers {
  ...
  async getAgent(args: { identity: string }): Promise<CallToolResult> {
    try {
      ...
      if (!rows.length) {
        return { content: [{ type: "text", text: `Agent ${args.identity} not found.` }] };
      }
  ...
  async registerAgent(args: {
    ...
    try {
      const { rows } = await query(
        ...
        [args.identity, args.agent_type || null, args.role || null, args.skills ? JSON.parse(args.skills) : null],
      );
      return { content: [{ type: "text", text: `Agent registered: ${rows[0].agent_identity} (${rows[0].role})` }] };
    } catch (err) {
      return errorResult("Failed to register agent", err);
    }
  }

  async listTeams(args: {}): Promise<CallToolResult> {
    try {
      const { rows } = await query(`SELECT * FROM team ORDER BY team_name`);
      if (!rows.length) {
        return { content: [{ type: "text", text: "No teams found." }] };
      }
      const lines = rows.map((r) => `${r.team_name} (${r.team_type}) — ${r.status}`);
      return { content: [{ type: "text", text: lines.join("\n") }] };
    } catch (err) {
  ...
  async createTeam(args: { name: string; team_type?: string }): Promise<CallToolResult> {
    try {
      const { rows } = await query(
        `INSERT INTO team (team_name, team_type) VALUES ($1, $2) RETURNING *`,
        [args.name, args.team_type || null],
      );
      return { content: [{ type: "text", text: `Team created: ${rows[0].team_name}` }] };
    } catch (err) {
      return errorResult("Failed to create team", err);
    }
  }

  async addTeamMember(args: { team_name: string; agent_identity: string; role?: string }): Promise<CallToolResult> {
    try {
      const teamRes = await query(`SELECT id FROM team WHERE team_name = $1`, [args.team_name]);
      if (!teamRes.rows.length) {
        return { content: [{ type: "text", text: `Team ${args.team_name} not found.` }] };
      }
      const agentRes = await query(`SELECT id FROM agent_registry WHERE agent_identity = $1`, [args.agent_identity]);
      if (!agentRes.rows.length) {
        return { content: [{ type: "text", text: `Agent ${args.agent_identity} not found.` }] };
      }
      ...
      return { content: [{ type: "text", text: `${args.agent_identity} added to ${args.team_name}` }] };
    } catch (err) {
