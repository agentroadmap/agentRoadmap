...
const DraftsList: React.FC<DraftsListProps> = ({ onEditProposal, onNewDraft }) => {
  ...
  const loadDrafts = async () => {
    try {
      ...
      // Sort drafts by ID descending (newest first) - same as ProposalList
      const sortedDrafts = [...draftsData].sort((a, b) => {
        // Extract numeric part from proposal IDs (proposal-1, proposal-2, etc.)
        const idA = parseInt(a.id.replace('proposal-', ''), 10);
        const idB = parseInt(b.id.replace('proposal-', ''), 10);
  ...
  return (
    <div className="container mx-auto px-4 py-8 transition-colors duration-200">
      <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Draft Proposals</h1>
        ...
        {drafts.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Draft proposals will appear here before they're promoted to the main roadmap.</p>
          </div>
        ) : (
            ...
            {drafts.map((draft) => (
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1 cursor-pointer" onClick={() => onEditProposal(draft)}>
