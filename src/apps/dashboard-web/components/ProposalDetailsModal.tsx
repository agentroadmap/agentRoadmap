...
import type { AcceptanceCriterion, Directive, Proposal } from "../../../shared/types";
...
export const ProposalDetailsModal: React.FC<Props> = ({
  ...
  const [finalSummary, setFinalSummary] = useState(proposal?.finalSummary || "");
  const [criteria, setCriteria] = useState<AcceptanceCriterion[]>(proposal?.acceptanceCriteriaItems || []);
  const resolveDirectiveToId = useCallback((value?: string | null): string => {
    ...
    const activeTitleMatches = activeDirectives.filter((directive) => directive.title.trim().toLowerCase() === key);
    ...
    const archivedTitleMatches = archivedDirectives.filter((directive) => directive.title.trim().toLowerCase() === key);
    if (archivedTitleMatches.length === 1) {
      return archivedTitleMatches[0]?.id ?? normalized;
    }
    return normalized;
  }, [directiveEntities, archivedDirectiveEntities]);
  const resolveDirectiveLabel = useCallback((value?: string | null): string => {
    const normalized = (value ?? "").trim();
    if (!normalized) return "";
    const key = normalized.toLowerCase();
    const aliasKeys = new Set<string>([key]);
    ...
    const allDirectives = [...(directiveEntities ?? []), ...(archivedDirectiveEntities ?? [])];
    const idMatch = findIdMatch(allDirectives);
    if (idMatch) {
      return idMatch.title;
    }
    const titleMatches = allDirectives.filter((directive) => directive.title.trim().toLowerCase() === key);
  ...
  const [dependencies, setDependencies] = useState<string[]>(proposal?.dependencies || []);
  const [references, setReferences] = useState<string[]>(proposal?.references || []);
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (mode === "preview" && isReachedStatus && (e.key.toLowerCase() === "c") && !e.metaKey && !e.ctrlKey && !e.altKey) {
        e.preventDefault();
        e.stopPropagation();
        void handleComplete();
      }
  ...
  }, [mode, title, description, plan, notes, finalSummary, criteria, status]);
  ...
  useEffect(() => {
    ...
    apiClient.fetchProposals().then(setAvailableProposals).catch(() => setAvailableProposals([]));
  }, [proposal, isOpen, isCreateMode, isDraftMode, availableStatuses]);
  ...
  const handleToggleCriterion = async (index: number, checked: boolean) => {
    ...
    setCriteria(next);
    try {
      await apiClient.updateProposal(proposal.id, { acceptanceCriteriaItems: next });
      if (onSaved) await onSaved();
    } catch (err) {
      // rollback
  ...
  const handleInlineMetaUpdate = async (updates: InlineMetaUpdatePayload) => {
    ...
    if (updates.directive !== undefined) setDirective((updates.directive ?? "") as string);
  ...
  // labels handled via ChipInput; no textarea parsing
	...
	const handleComplete = async () => {
		if (!proposal) return;
		if (!window.confirm("Complete this proposal? It will be moved to the completed folder.")) return;
		try {
  ...
  const handleArchive = async () => {
    ...
    if (!window.confirm(`Are you sure you want to archive "${proposal.title}"? This will move the proposal to the archive folder.`)) return;
    onArchive();
    onClose();
  };
    ...
    <Modal
      ...
      actions={
        <div className="flex items-center gap-2">
		          {isReachedStatus && mode === "preview" && !isCreateMode && !isFromOtherBranch && (
		            <button
      ...
      {isFromOtherBranch && (
        <div className="mb-4 flex items-center gap-2 px-4 py-3 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700 rounded-lg text-amber-800 dark:text-amber-200">
          ...
          <div className="flex-1">
            <span className="font-medium">Read-only:</span> This proposal exists in the <span className="font-semibold">{proposal?.branch}</span> branch. Switch to that branch to edit it.
          </div>
        </div>
      )}
              ...
              {mode === "preview" ? (
                ...
                <div className="border border-gray-200 dark:border-gray-700 rounded-md">
                  <MDEditor
                    ...
                    data-color-mode={theme}
                    textareaProps={{
                      placeholder: "PR-style summary of what was implemented (write when proposal is complete)",
                    }}
                  />
                </div>
              )}
          ...
          {/* Dates */}
	          {proposal && (
	            <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3 text-xs text-gray-600 dark:text-gray-300 space-y-1">
	              <div><span className="font-semibold text-gray-800 dark:text-gray-100">Created:</span> <span className="text-gray-700 dark:text-gray-200">{formatStoredUtcDateForDisplay(proposal.createdDate)}</span></div>
	              {proposal.updatedDate && (
	                <div><span className="font-semibold text-gray-800 dark:text-gray-100">Updated:</span> <span className="text-gray-700 dark:text-gray-200">{formatStoredUtcDateForDisplay(proposal.updatedDate)}</span></div>
	              )}
	            </div>
	          )}
            ...
            <select
              ...
              value={directiveSelectionValue}
				onChange={(e) => {
					...
					setDirective(value);
					handleInlineMetaUpdate({ directive: value.trim().length > 0 ? value : null });
				}}
              disabled={isFromOtherBranch}
            >
              <option value="">No directive</option>
              {!hasDirectiveSelection && directiveSelectionValue ? (
                <option value={directiveSelectionValue}>{resolveDirectiveLabel(directiveSelectionValue)}</option>
              ) : null}
