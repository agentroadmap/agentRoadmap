                ...
                steps: [
                    {
                        ...
                        command: "systemctl status agenthive-mcp.service || sudo systemctl restart agenthive-mcp.service",
                    },
                    {
                        description: "Check MCP URL configuration",
                        command: "echo $HIVE_MCP_URL || cat ~/.hive/config.json | jq '.mcp_url'",
                    },
                ],
            },
        };
                ...
                steps: [
                    {
                        ...
                        command: "pg_isready -h 127.0.0.1 -p 5432 || systemctl status postgresql",
                    },
                    {
 ...
 */
async function checkSchemaMigrated() {
 ...
 */
async function checkPgPoolHealth() {
    const pool = getDbPool();
    if (!pool) {
        return {
            id: "pg_pool_health",
 ...
 */
async function checkNoZombieListen() {
    const pool = getDbPool();
    if (!pool) {
        return {
            id: "no_zombie_listen",
 ...
 */
async function checkOperatorAudit() {
    const pool = getDbPool();
    if (!pool) {
        return {
            id: "operator_audit",
 ...
 */
async function checkStateNames() {
 ...
 */
async function checkProjectBootstrapped() {
    const pool = getDbPool();
    if (!pool) {
        return {
 ...
 */
async function checkNoStuckProposals() {
    const pool = getDbPool();
    if (!pool) {
        return {
 ...
 */
async function checkDispatchesNotAllFailed() {
    const pool = getDbPool();
    if (!pool) {
        return {
 ...
 */
export async function getRemediation(checkId) {
