...
import { loadRecipes, getRecipeById, validateRecipe, } from "./recipes.js";
describe("recipes", () => {
    let recipes;
    before(async () => {
    ...
    describe("validateRecipe", () => {
        it("should validate all bundled recipes as correct", async () => {
            ...
            loaded.forEach((recipe) => {
                ...
                assert.strictEqual(error, undefined, `Recipe "${recipe.id}" should be valid: ${error}`);
            });
        });
        it("should detect invalid domain in hive command", () => {
            ...
            assert(error.includes("invalid-domain"), "Error should mention the invalid domain");
        });
        ...
        it("should allow external commands (git, npm, jq)", () => {
            ...
            externalCommands.forEach((command) => {
                ...
                const error = validateRecipe(recipe);
                assert.strictEqual(error, undefined, `External command "${command}" should be allowed`);
            });
        });
        ...
        it("should allow pipes and logical operators", () => {
            const recipe = {
                ...
                steps: [
                    {
                        command: "hive proposal list --format json | jq '.data[] | select(.state == \"DRAFT\")'",
    ...
    describe("recipe structure", () => {
        ...
        it("should have unique recipe IDs", () => {
            ...
            assert.strictEqual(uniqueIds.size, ids.length, "All recipe IDs should be unique");
        });
        it("should have recipe IDs that are slug-style (lowercase, hyphens, no spaces)", () => {
            recipes.forEach((recipe) => {
                assert(/^[a-z0-9-]+$/.test(recipe.id), `Recipe ID "${recipe.id}" should be slug-style`);
            });
        });
        it("should have at least 3 steps in each recipe", () => {
            recipes.forEach((recipe) => {
                assert(recipe.steps.length >= 3, `Recipe "${recipe.id}" should have at least 3 steps`);
            });
        });
        ...
        it("should have valid on_error values (abort or continue)", () => {
            recipes.forEach((recipe) => {
                recipe.steps.forEach((step) => {
                    if (step.on_error) {
                        assert(["abort", "continue"].includes(step.on_error), `on_error should be "abort" or "continue", got "${step.on_error}"`);
                    }
                });
            });
        });
    ...
    describe("recipe semantics", () => {
        it("claim-and-develop recipe should capture necessary context", () => {
            ...
            assert(commands.some((c) => c.includes("context")), "Should have context step");
            assert(commands.some((c) => c.includes("proposal")), "Should have proposal step");
            assert(commands.some((c) => c.includes("claim")), "Should have claim step");
            assert(commands.some((c) => c.includes("maturity")), "Should have maturity step");
        });
        it("audit-before-commit recipe should check scan/lint/test before commit", () => {
            const recipe = getRecipeById("audit-before-commit");
            assert(recipe !== undefined);
            const commands = recipe?.steps.map((s) => s.command) || [];
            assert(commands.some((c) => c.includes("scan")), "Should have scan step");
            assert(commands.some((c) => c.includes("lint")), "Should have lint step");
            assert(commands.some((c) => c.includes("npm test")), "Should have npm test step");
            assert(commands.some((c) => c.includes("git commit")), "Should have git commit step");
        ...
        it("doctor-self-test recipe should include doctor command", () => {
            ...
            assert(recipe !== undefined);
            const commands = recipe?.steps.map((s) => s.command) || [];
            assert(commands.some((c) => c.includes("hive doctor")), "Should have doctor step");
        });
        it("capture-defect recipe should transition to REVIEW state", () => {
            const recipe = getRecipeById("capture-defect");
            assert(recipe !== undefined);
            const commands = recipe?.steps.map((s) => s.command) || [];
            assert(commands.some((c) => c.includes("transition") && c.includes("REVIEW")), "Should transition to REVIEW");
        });
        it("project-bootstrap recipe should include db create and agency register", () => {
            const recipe = getRecipeById("project-bootstrap");
            assert(recipe !== undefined);
            const commands = recipe?.steps.map((s) => s.command) || [];
            assert(commands.some((c) => c.includes("project init")), "Should have project init step");
            assert(commands.some((c) => c.includes("db create")), "Should have db create step");
            assert(commands.some((c) => c.includes("agency register")), "Should have agency register step");
        });
    });
});
