...
const BUNDLED_RECIPES = [
    {
        ...
        when_to_use: "Agent has available capacity and wants to start a new piece of work",
        steps: [
            {
            ...
            },
            {
                ...
                description: "Get next claimable proposal ranked by gate-readiness score",
            ...
            {
                command: "hive proposal show ${proposal_id} --include all --format json",
        ...
        terminal_state: "Proposal claimed with active lease (4h), maturity=active, full state loaded",
    },
    {
        ...
        steps: [
            ...
            },
            {
                ...
                description: "Scan for hardcoding, secrets, TODO TODOs added in this branch",
                reads: ["finding_count"],
                expects: "ok: true",
            },
            {
                command: "hive scan --since HEAD --format json | jq '.data.findings | length'",
                description: "Count scan findings; if >0, show them before proceeding",
                on_error: "continue",
            },
            {
            ...
            {
                command: "git add -A && git commit -m 'commit: scan/lint/test pass'",
                description: "Commit changes only if all checks passed",
                expects: "exit 0",
            },
        ],
        terminal_state: "Local changes committed, no new scan/lint findings, test suite passes",
    },
    {
        id: "capture-defect",
        ...
        steps: [
            ...
            {
                command: "hive proposal create --type issue --title '${DEFECT_TITLE}' --description '${DEFECT_DESC}' --format json",
                description: "Create an issue proposal in DRAFT state",
                writes: ["proposal_id"],
                expects: "ok: true",
            },
            {
                command: "hive proposal ac add ${proposal_id} --description 'Bug is reproducible' --description 'Root cause identified' --description 'Fix tested' --format json",
                description: "Add acceptance criteria for the defect resolution",
                expects: "ok: true",
            },
            {
                command: "hive proposal transition ${proposal_id} REVIEW --reason 'Ready for triage review'",
                description: "Move to REVIEW state for team triage",
                expects: "ok: true",
            },
        ],
        terminal_state: "Defect issue created, AC defined, transitioned to REVIEW for triage",
    },
    {
        id: "operator-stop-runaway",
        title: "Stop a runaway dispatch or worker process",
        when_to_use: "A dispatch or worker is hung, consuming excessive budget, or misbehaving",
        steps: [
            {
                command: "hive context --format json",
            ...
            {
                command: "hive dispatch cancel ${dispatch_id} --reason 'Runaway: excessive resource usage' --format json",
                description: "Force-cancel the runaway dispatch",
                expects: "ok: true",
            },
            {
                command: "hive proposal escalate ${proposal_id} --reason 'Dispatch was cancelled due to runaway behavior' --format json",
    ...
    },
    {
        ...
        when_to_use: "Proposal has been in same state >7 days without maturity change",
        steps: [
            {
                command: "hive proposal show ${PROPOSAL_ID} --include all --format json",
            ...
            {
                command: "hive proposal ac verify ${PROPOSAL_ID} --format json | jq '.data | {total, verified, unverified}'",
                description: "Check which acceptance criteria are blocking; count verified vs pending",
                expects: "ok: true",
            },
            {
                command: "hive proposal dependencies ${PROPOSAL_ID} --format json | jq '.data[] | select(.status != \"satisfied\")'",
                description: "List unsatisfied (blocking) dependencies",
                expects: "ok: true",
            },
            {
                command: "hive workflow next-state ${PROPOSAL_ID} --format json | jq '{legal_next_states, blockers, why_blocked}'",
                description: "Check what would unblock the proposal for transition",
                expects: "ok: true",
            },
        ],
        terminal_state: "Agent understands blockers (AC, deps, gate decision) and next action",
    },
    {
        ...
        steps: [
            {
                command: "hive project init --name '${PROJECT_NAME}' --git-root ${GIT_ROOT} --format json",
            ...
            {
                command: "hive agency register --project ${project_id} --identity 'hermes/agency-${PROJECT_NAME}' --capabilities 'propose,develop,review' --format json",
                description: "Register agency for the project",
                writes: ["agency_id"],
                expects: "ok: true",
            },
            {
                command: "hive config set --scope project --key initial_budget_usd --value '1000' --project ${project_id}",
        ...
        terminal_state: "Project initialized, agency registered, budget set, health check passes",
    },
    {
        ...
        steps: [
            ...
            {
                command: "hive proposal create --type feature --title '${PARENT_TITLE}' --format json",
                description: "Create parent proposal (umbrella task)",
                writes: ["parent_proposal_id"],
            },
            {
                command: "hive proposal create --type feature --title '${SUBTASK_1}' --depends-on ${parent_proposal_id} --format json",
                description: "Create first subtask with dependency on parent (repeat for each subtask)",
                writes: ["subtask_1_id"],
            },
            {
                command: "hive agency list --filter status=AVAILABLE --format jsonl --limit 3",
                description: "Find available agencies to claim subtasks",
                reads: ["agency_ids"],
            },
            {
                command: "hive proposal claim ${subtask_1_id} --duration 8h --agency ${agency_1} --format json",
                description: "Claim first subtask for first agency (repeat for each)",
                expects: "ok: true",
            },
        ],
        terminal_state: "Parent and subtasks created; each subtask claimed by different agency",
    },
    {
        id: "doctor-self-test",
        title: "Run system health checks and apply suggested remediations",
        when_to_use: "Verifying the system is healthy before starting work or after an incident",
        steps: [
            {
            ...
            {
                command: "hive doctor --format json | jq '.data.issues[] | select(.code == \"ORPHAN_LEASE\")'",
                description: "Check for orphan leases that need cleanup",
                on_error: "continue",
            },
            {
                command: "hive doctor --remediate ORPHAN_LEASE --format json | jq '.data.remediation.steps[] | .command'",
                description: "Get suggested remediation commands for orphan leases",
                on_error: "continue",
            },
            {
                command: "hive doctor --format json | jq 'if .data.overall_status == \"healthy\" then \"OK: System is healthy\" else \"WARNING: Issues detected\" end'",
                description: "Final status check",
                expects: "OK or WARNING",
            },
        ],
        terminal_state: "System health verified; any known issues documented with remediation suggestions",
    },
];
/**
 * Load all recipes.
