...
export interface FilterHeaderOptions {
	...
	initialFilters?: Partial<FilterProposal>;
	visibleFilters?: FilterControlId[];
	onFilterChange: (filters: FilterProposal) => void;
...
export class FilterHeader {
	...
	private proposal: FilterProposal;
	...
	/**
	 * Get current filter proposal
	 */
	getFilters(): FilterProposal {
		return { ...this.proposal };
	}

	/**
	 * Update filter proposal externally
	 */
	setFilters(filters: Partial<FilterProposal>): void {
		...
		if (filters.priority !== undefined) {
			this.proposal.priority = filters.priority;
			this.updatePriorityButton();
		}
		if (filters.labels !== undefined) {
			this.proposal.labels = filters.labels;
	...
	private commitSearchValue(): void {
		...
		if (value !== undefined && value !== this.proposal.search) {
			this.proposal.search = String(value);
			this.emitFilterChange();
		}
	}
	...
	private buildFilterItem(item: FilterItem, x: number, y: number, width: number): void {
		...
		switch (item.id) {
			...
			case "priority":
				...
				break;
			case "directive":
				this.buildPopupButton("directive", controlX, y, controlWidth);
				break;
			case "labels":
	...
	private updateDirectiveButton(): void {
		if (!this.directiveButton) return;
		this.directiveButton.setContent(this.getPopupButtonContent("directive"));
	}
	...
	private emitFilterChange(): void {
		this.options.onFilterChange({ ...this.proposal });
	}
}
