 ...
 * - Vertical layout: each sequence has a header and its proposals listed below.
 * - Exit with 'q' or 'Esc'.
 */
export async function runSequencesView(
	data: { unsequenced: Proposal[]; sequences: Sequence[] },
	core?: Core,
): Promise<void> {
	...
	// Build bordered blocks for unsequenced and sequences, and individual proposal lines (for selection)
	let y = 0;
	type ProposalLine = {
		node: BoxInterface;
		globalIndex: number;
	...
	if (data.unsequenced.length > 0) {
		...
		for (let t = 0; t < data.unsequenced.length; t++) {
			const lineTop = t + 1;
			const proposal = data.unsequenced[t];
	...
	for (let s = 0; s < data.sequences.length; s++) {
		...
		if (!seq) continue;
		const proposalsSorted = [...seq.proposals].sort((a, b) => {
		...
		const h = Math.max(4, proposalsSorted.length + 4);
		const block = box({
			parent: container,
			top: y,
			left: 0,
		...
		for (let t = 0; t < proposalsSorted.length; t++) {
			// Render inside bordered content area
			const lineTop = t + 1;
			const proposal = proposalsSorted[t];
			if (!proposal) continue;
			const node = box({
				parent: block,
				top: lineTop,
	...
	screen.key(["m", "M"], () => {
		if (popupOpen) return;
		moveMode = !moveMode;
		// Default target is the selected proposal's current sequence
		const item = proposalLines.find((t) => t.globalIndex === selected);
		if (item) {
			if (item.seqIdx === -1) {
