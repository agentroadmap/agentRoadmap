...
export type ProposalSummary = Pick<
  ProposalRow,
  'id' | 'display_id' | 'type' | 'title' | 'status' | 'priority' | 'maturity' | 'tags' | 'audit' | 'created_at'
> & {
...
export async function listProposals(
  filters?: { status?: string; type?: string; maturity_stage?: string },
): Promise<ProposalRow[]> {
...
export async function resolveProposalId(identifier: string | number): Promise<number | null> {
 ...
 */
export async function getProposalSummary(identifier: string | number): Promise<ProposalSummary | null> {
  const numericId = typeof identifier === 'number' ? identifier : Number.parseInt(identifier, 10);
 ...
 */
export async function createProposal(input: ProposalCreateInput, authorIdentity: string): Promise<ProposalRow> {
  const { rows } = await query<ProposalRow>(
...
export async function transitionProposal(
  ...
  if (rowCount === 0) {
    throw new Error(`Transition ${fromState} → ${toState} not allowed for this proposal's workflow`);
  }
 ...
 */
export async function getValidTransitions(proposalId: number): Promise<{ from_state: string; to_state: string; labels?: string[]; allowed_roles?: string[] }[]> {
...
export async function getActiveLeases(proposalId?: number) {
  ...
  const { rows } = await query(
    `SELECT * FROM roadmap.v_active_leases`,
  );
  return rows;
}
...
export async function listDependencies(proposalIds?: number[]): Promise<ProposalDependency[]> {
  const params: unknown[] = [];
...
export async function replaceAcceptanceCriteria(
  proposalId: number,
  criteria: Array<{ item_number: number; criterion_text: string; status?: string }>,
): Promise<void> {
  const client = await getPool().connect();
  try {
    await client.query("BEGIN");
...
export async function releaseExpiredLeases(before = new Date()): Promise<number[]> {
 ...
 */
export async function proposalSummary(): Promise<{ status: string; count: number }[]> {
...
export async function deleteProposal(identifier: string | number): Promise<boolean> {
  const proposalId = await resolveProposalId(identifier);
  if (proposalId === null) {
    return false;
  }
