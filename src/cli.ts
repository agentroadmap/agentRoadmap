...
import { DEFAULT_CLAIM_DURATION_MINUTES, DEFAULT_DIRECTORIES } from "./constants/index.ts";
...
import { formatCompactProposalListLine } from "./formatters/proposal-list-plain-text.ts";
...
proposalCmd
	...
	.action(async (proposalId: string | undefined, options) => {
		...
		try {
			...
			// Sync to SDB if status changed
			if (editArgs.status) {
				try {
					const { transitionProposalInSDB } = await import("./mcp/tools/proposals/sdb-transition.ts");
					const sdbResult = await transitionProposalInSDB(canonicalId, editArgs.status, "cli", "Updated via CLI");
					if (sdbResult.success) {
						console.error(`[SDB] Synced ${canonicalId} → ${editArgs.status}`);
					}
				} catch (e) {
					console.error(`[SDB] Sync skipped: ${e instanceof Error ? e.message.slice(0, 60) : e}`);
				}
			}
		} catch (error) {
			console.error(error instanceof Error ? error.message : String(error));
...
function addBoardOptions(cmd: Command) {
	return cmd
		...
		.option("-s, --source <source>", "data source (spacetimedb)", "spacetimedb")
		.option("-n, --namespace <name>", "project namespace (for spacetimedb)")
		.option("--plain", "use plain text output instead of interactive UI");
}
...
async function handleBoardView(options: { 
	...
	const source = options.source || "sdb";

	if (options.plain || !process.stdout.isTTY) {
		let proposals: Proposal[];
		if (source === "file") {
			proposals = await core.loadProposals();
		} else {
			// Default: SpacetimeDB
	...
	await runUnifiedView({
		...
		proposalsLoader: async (updateProgress) => {
			...
			if (source === "file") {
				proposals = await core.loadProposals((msg) => {
					updateProgress(msg);
				});
				directiveEntities = await core.filesystem.listDirectives();
			} else {
				updateProgress("Connecting to SpacetimeDB...");
				const { getSdbData } = await import('./core/storage/sdb-sdk-loader.ts');
			...
			const [archivedDirectives] = await Promise.all([
				source === "file" ? core.filesystem.listArchivedDirectives() : Promise.resolve([]),
			]);
