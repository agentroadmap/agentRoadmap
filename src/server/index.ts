...
export class RoadmapServer {
	...
	private async handleGetProposalNotes(proposalId: string, req: Request): Promise<Response> {
		try {
			...
			const sdbConfig = getSdbConfigSync();
			const dbName = sdbConfig.dbName;
			const server = sdbConfig.host === "127.0.0.1" ? "local" : `http://${sdbConfig.host}:${sdbConfig.port}`;

			// Use SDB discussion table (S141 or V2.2)
			const table = dbName === "roadmap2" ? "proposal_discussion" : "discussion";
			const idColumn = dbName === "roadmap2" ? "proposal_id" : "step_id";
			const authorColumn = dbName === "roadmap2" ? "agent_identity" : "agent_id";
			
			let query = `SELECT id, ${idColumn}, ${authorColumn}, content, note_type, created_at FROM ${table} WHERE ${idColumn} = '${proposalId}'`;
			if (noteType) {
				query += ` AND note_type = '${noteType}'`;
			}
			const result = execSync(`spacetime sql --server ${server} ${dbName} "${query}"`, { encoding: "utf8" });
			const lines = result.trim().split("\n").filter((l: string) => !l.includes("WARNING"));
			if (lines.length < 2) return Response.json({ notes: [], proposal_id: proposalId });
			const headers = lines[0].split("|").map((h: string) => h.trim()).filter(Boolean);
			const notes = lines.slice(1).filter((l: string) => l.includes("|")).map((line: string) => {
				const values = line.split("|").map((v: string) => v.trim().replace(/"/g, ""));
				const obj: any = {};
				headers.forEach((h: string, i: number) => { obj[h] = values[i]; });
				return obj;
			}).filter((n: any) => n.id && !n.id.includes("-"));
