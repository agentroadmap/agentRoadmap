...
/** In-memory message store with MCP-backed cross-agent sync. */
 ...
 * Combines local memory with MCP-backed channel reads for cross-agent consistency.
 */
export async function getMessages(filter: MessageFilter): Promise<AgentMessage[]> {
  ...
  // Try to also read from the shared MCP channel for cross-agent messages
...
export function acknowledgeMessage(messageId: string): boolean {
  ...
  if (index >= 0) {
    // In a fuller implementation, this would persist acknowledgement state.
    return true;
  }
  return false;
}
