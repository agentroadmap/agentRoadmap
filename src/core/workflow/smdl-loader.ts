...
export async function materializeWorkflow(smdl: SMDLRoot): Promise<{ templateId: number; stages: number; transitions: number; roles: number }> {
  ...
  for (const stage of wf.stages) {
    ...
    if (workflowGateOverride) {
      gatingConfig.decision_gate = { ...gatingConfig.decision_gate, ...workflowGateOverride };
    }
    const gatingConfigJson = Object.keys(gatingConfig).length > 0 ? JSON.stringify(gatingConfig) : null;
    ...
    await query(
      ...
      [templateId, stage.name, stage.order, stage.maturity_gate ?? wf.default_maturity_gate ?? 2, stage.requires_ac ?? false, gatingConfigJson],
    );
    stagesCount++;
  }
  ...
  // RFC-5
  {
    workflow: {
      ...
      stages: [
        ...
        { name: 'DRAFT', order: 2, description: 'AI research and enhancement', auto_transitions: { on_mature: 'REVIEW' }, decision_gate: { evaluator: 'auto' } },
        { name: 'REVIEW', order: 3, description: 'Formal review, define acceptance criteria', requires_ac: true, quorum: { required_count: 2, required_roles: ['PM', 'Architect'], veto_power: true }, auto_transitions: { on_mature: 'DEVELOP' }, decision_gate: { evaluator: 'auto', escalate_to_user: { cost_usd: 0.50, impact_score: 20, operator: 'AND' } } },
