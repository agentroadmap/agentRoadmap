...
export const STRUCTURED_SECTION_KEYS: Record<StructuredSectionKey, string> = {
...
const SECTION_CONFIG: Record<StructuredSectionKey, SectionConfig> = {
	...
	auditNotes: { title: "Audit Notes", markerId: "AUDIT_NOTES" },
	finalSummary: { title: "Final Summary", markerId: "FINAL_SUMMARY" },
	scopeSummary: { title: "Scope Summary", markerId: "SCOPE_SUMMARY" },
	proof: { title: "Proof of Arrival", markerId: "PROOF" },
...
const VERIFICATION_STATEMENTS_DEFINITION: ChecklistSectionDefinition = {
	sectionHeader: VERIFICATION_STATEMENTS_SECTION_HEADER,
	title: VERIFICATION_STATEMENTS_TITLE,
	beginMarker: VERIFICATION_STATEMENTS_BEGIN_MARKER,
	endMarker: VERIFICATION_STATEMENTS_END_MARKER,
};
...
function getStructuredSectionRanges(content: string): SectionRange[] {
	...
	// Collect ALL sentinels first across ALL keys
	...
	// Then collect legacy ranges, but ONLY if they don't overlap with any sentinel
	for (const key of SECTION_INSERTION_ORDER) {
...
export function updateStructuredSections(content: string, sections: SectionValues): string {
	...
	const existingSections = getStructuredSections(src);
	const mergedSections: SectionValues = {
		...existingSections,
		...sections,
	};

	let working = src;
	// Strip all existing instances of structured sections first
	for (const key of SECTION_INSERTION_ORDER) {
	...
	const description = mergedSections.description?.trim() || "";
	const plan = mergedSections.implementationPlan?.trim() || "";
	const notes = mergedSections.implementationNotes?.trim() || "";
	const auditNotes = mergedSections.auditNotes?.trim() || "";
	const finalSummary = mergedSections.finalSummary?.trim() || "";
	const scopeSummary = mergedSections.scopeSummary?.trim() || "";
	const proof = mergedSections.proof?.trim() || "";
	...
	if (plan) {
		...
		if (!res.inserted) {
			res = insertAfterSection(tail, VERIFICATION_STATEMENTS_TITLE, planBlock);
		}
		if (!res.inserted) {
	...
	// Description is special: always at the very top
	let output = tail;
	if (description) {
...
function parseOldChecklistFormat(content: string, definition: ChecklistSectionDefinition): AcceptanceCriterion[] {
	...
	for (const line of lines) {
		...
		if (checkboxMatch?.[1] && checkboxMatch?.[2]) {
			const { text, role, evidence } = parseStructuredText(checkboxMatch[2]);
			criteria.push({
...
function parseChecklist(content: string, definition: ChecklistSectionDefinition): AcceptanceCriterion[] {
	...
	for (const line of lines) {
		...
		if (match?.[1] && match?.[2] && match?.[3]) {
			const { text, role, evidence } = parseStructuredText(match[3]);
			criteria.push({
				checked: match[1] === "x",
				text,
...
function formatCriterionText(criterion: AcceptanceCriterion): string {
	let text = criterion.text;
	if (criterion.role) {
		text = `[${criterion.role}] ${text}`;
	}
	if (criterion.evidence) {
		text = `${text} (evidence: ${criterion.evidence})`;
	}
	return text;
}
	...
	if (sourceLines.length > 0) {
		for (const line of sourceLines) {
			...
			if (checkboxMatch) {
				...
				const newLine = `- [${criterion.checked ? "x" : " "}] #${nextNumber++} ${formatCriterionText(criterion)}`;
				lines.push(newLine);
			} else {
				lines.push(line);
			}
...
export function parseStructuredText(fullText: string): { text: string; role?: string; evidence?: string } {
	...
	// Extract evidence: text (evidence: type)
	const evidenceMatch = text.match(/^(.*)\s*\(evidence:\s*([^)]+)\)$/);
	if (evidenceMatch) {
		text = evidenceMatch[1].trim();
		evidence = evidenceMatch[2].trim();
	}
...
/* biome-ignore lint/complexity/noStaticOnlyClass: Utility methods grouped for clarity */
export class VerificationProposalmentsManager {
	static readonly BEGIN_MARKER = VERIFICATION_STATEMENTS_BEGIN_MARKER;
	...
	static addCriteria(content: string, newCriteria: string[]): string {
		...
		return VerificationProposalmentsManager.updateContent(content, existing);
	}
	...
	static removeCriterionByIndex(content: string, index: number): string {
		...
		const filtered = criteria.filter((c) => c.index !== index);
		if (filtered.length === criteria.length) {
			throw new Error(`Verification proposalment #${index} not found`);
		}
		const renumbered = filtered.map((c, i) => ({ ...c, index: i + 1 }));
	...
	static checkCriterionByIndex(content: string, index: number, checked: boolean): string {
		...
		criterion.checked = checked;
		return VerificationProposalmentsManager.updateContent(content, criteria);
	}
	...
	static migrateToStableFormat(content: string): string {
		...
		if (
			content.includes(VerificationProposalmentsManager.BEGIN_MARKER) &&
			content.includes(VerificationProposalmentsManager.END_MARKER)
		) {
			return content;
		}
		return VerificationProposalmentsManager.updateContent(content, criteria);
