...
const DependencyInput: React.FC<DependencyInputProps> = ({ value, onChange, availableProposals, currentProposalId, label = 'Dependencies', disabled }) => {
  ...
  const getProposalDisplay = (proposalId: string) => {
  ...
  useEffect(() => {
    if (inputValue.trim()) {
      const filtered = availableProposals.filter(proposal => 
        proposal.id !== currentProposalId && // Don't suggest current proposal
        !value.includes(proposal.id) && // Don't suggest already added proposals
        (proposal.id.toLowerCase().includes(inputValue.toLowerCase()) ||
         proposal.title.toLowerCase().includes(inputValue.toLowerCase()))
      );
  ...
  const addDependency = (proposalId: string) => {
    if (disabled) return;
          ...
          {value.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-2">
              {value.map((proposalId, index) => (
                ...
                >
                  <span className="truncate max-w-[16rem] sm:max-w-[20rem] md:max-w-[24rem]">{getProposalDisplay(proposalId)}</span>
	                  {!disabled && (
	                    <button
	                      type="button"
        ...
        {suggestions.length > 0 && (
          <div className="absolute z-10 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md shadow-lg max-h-64 overflow-auto overscroll-contain transition-colors duration-200">
	            {suggestions.map((proposal, index) => (
	              ...
	              >
                <div className="font-medium text-gray-900 dark:text-white">{proposal.id}</div>
                <div className="text-gray-600 dark:text-gray-300 break-words whitespace-normal">{proposal.title}</div>
              </button>
            ))}
          </div>
        )}
