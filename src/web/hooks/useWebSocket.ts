/**
 * WebSocket Client Hook for Board ↔ SpacetimeDB
 *
 * Subscribes to the WebSocket bridge for real-time proposal, agent,
 * and channel updates from the roadmap2 database.
 */
...
export function useWebSocket(url: string = 'ws://localhost:3001'): UseWebSocketReturn {
  ...
  const connect = useCallback(() => {
    ...
    ws.onopen = () => {
      console.log('[WS] Connected to SpacetimeDB bridge');
      setConnected(true);
    ...
    ws.onmessage = (event) => {
      try {
        ...
        switch (msg.type) {
          ...
          case 'sync':
            // SpacetimeDB subscription event — re-fetch
