	...
	for (const line of lines) {
		...
		if (structuredMatch) {
			proofItems.push({
				...
				value: structuredMatch[2],
				summary: structuredMatch[3],
			});
		} else {
			// Fallback: treat as link if it looks like a URL/hash, otherwise observation
			const isUrl = line.startsWith("http") || line.includes("/");
			proofItems.push({
				type: isUrl ? "link" : "observation",
				value: line,
			});
		}
	}
...
export function parseProposal(content: string): Proposal {
	...
	return {
		...
		claim: frontmatter.claim
			? {
					agent: String((frontmatter.claim as any).agent || ""),
					created: normalizeDate((frontmatter.claim as any).created),
					expires: normalizeDate((frontmatter.claim as any).expires),
...
export function parseDirective(content: string): Directive {
	const { frontmatter, content: rawContent } = parseMarkdown(content);
