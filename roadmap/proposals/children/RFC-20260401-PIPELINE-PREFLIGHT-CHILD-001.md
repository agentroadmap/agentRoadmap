---
id: RFC-20260401-PIPELINE-PREFLIGHT-CHILD-001
display_id: RFC-20260401-PIPELINE-PREFLIGHT-CHILD-001
proposal_type: TECHNICAL
category: INFRA
domain_id: ENGINE
title: "Pipeline Pre-flight Verification Protocol"
status: Draft
maturity: 0
summary: "Define the MCP-driven pre-flight check protocol for code validation before submission to Postgres."
---

# Pipeline Pre-flight Verification Protocol

To finalize the **`pipeline/`** domain, here is the **Pre-flight Check** protocol. This is an MCP-driven script that a **Coder Agent** must run locally on its host machine (Mac, Windows, or Linux) before it is allowed to submit an RFC or a code promotion request to Postgres.
...
## Integration with the `workforce/` Domain
...
## The "Promotion" Transaction in Postgres
Once the pre-flight passes, the agent calls a **Promotion workflow action**.
