...
- **Postgres as state** — persistent team channels, work claiming, coordination
- **Bridge Pattern** — OpenClaw = radio (how), ClawHub = studio (what/why)

## Components to Build
- [ ] ClawHub Docker image (MCP + Postgres + Config Vault)
...
- All agents register with ClawHub Postgres

## Dependencies
- Fully isolated cubic (prerequisite)
- OpenClaw `openclaw-acp` CLI availability
- Postgres schema for agent registration + work claiming
