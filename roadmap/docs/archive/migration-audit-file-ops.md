# Migration Audit: File-Based Operations → SpacetimeDB Equivalents
**STATE-095 AC#1** | Created: 2026-03-25 14:50 UTC | Author: Carter

## Executive Summary

**35 core modules** use file-based I/O. Of these:
- **Tier 1 (Already migrated):** 3 modules → SpacetimeDB
...
## Tier 1: Already Migrated to SpacetimeDB ✅

| Module | File Ops | SDB Table | Status |
|--------|----------|-----------|--------|
| `state-storage.ts` | read/write state files | `states` | ✅ Live |
| `registry.ts` | agent registration | `agents` | ✅ Live |
| `cli-adapter.ts` | CLI → SDB bridge | `states`, `agents` | ✅ Live |

---

## Tier 2: Priority Migration to SpacetimeDB 🎯
...
| Module | File Ops | Proposed SDB Table | Data Shape |
|--------|----------|-------------------|------------|
...
| `pulse.ts` | Advisory generation | Computed from SDB data |
...
|--------|------------|-----------------|
| `knowledge-base.ts` | Read/write knowledge docs | Could move to SDB `knowledge` table |
...
| `issue-tracker.ts` | Issue JSON files | Could move to SDB `issues` table |
...
async function saveState(state: State) {
  // 1. Write to SpacetimeDB (primary)
  await spacetimedb.insert('states', state);
  
  // 2. Write to file (fallback)
  await writeFile(statePath(state.id), serialize(state));
  
  // 3. Verify consistency
  const dbState = await spacetimedb.query('states', state.id);
  assert(checksum(dbState) === checksum(state));
}
```

**Exit criteria for dual-write removal:**
- All Tier 2 modules verified on SDB for 7 days
...
- [ ] AC#3: Field-by-field mapping (.md frontmatter → SDB columns)
