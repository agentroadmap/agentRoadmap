...
4-week rollout from file-based to Postgres-primary architecture, with measurable success criteria at each phase.
...
│            │  │            │  │            │  │            │
│ Postgres Verify │  │ Dual-Write │  │ Postgres Primary│  │ File Deprec│
└────────────┘  └────────────┘  └────────────┘  └────────────┘
...
**Goal:** Verify Postgres is reliable for all read operations.
...
| 5 | Shadow reads: compare Postgres vs file results | System |
...
| 2 | Deploy claims table + workflow actions | Carter |
| 3 | Deploy transition workflow actions | Carter |
| 4 | Migrate messaging to Postgres (already done) | System |
| 5 | Deploy merge workflow actions | Carter |
| 6 | Add MCP tools for Postgres operations | Carter |
...
- [ ] All workflow workflow actions passing integration tests
- [ ] Zero degraded mode triggers

### Rollback Trigger
- Dual-write consistency <99% for >1 hour
- Any data loss (state in Postgres but not in files)
- Workflow workflow action failures >1%

---

## Phase 3: FLIP (Week 3)

**Goal:** Postgres becomes primary for reads, files become fallback.

| Day | Action | Owner |
|-----|--------|-------|
| 1 | Switch read primary to Postgres | Carter |
| 2 | Enable backfill: missing states auto-synced to files | System |
| 3 | Board UI subscription to Postgres | Carter |
| 4 | MCP state tools via Postgres | Carter |
| 5 | Cron jobs recreated (pub/sub model) | Carter |
| 6 | Monitor all operations via Postgres | System |
...
| Metric | Target | Measurement |
|--------|--------|-------------|
| Read primary: Postgres | 100% of reads | Read routing logs |
...
- [ ] Board UI rendering correctly from Postgres
- [ ] All cron jobs migrated and running
- [ ] MCP tools working via Postgres

### Rollback Trigger
- Fallback hits >5% (Postgres not serving most reads)
...
**Goal:** Filesystem becomes archival only, Postgres is source of truth.

| Day | Action | Owner |
|-----|--------|-------|
| 1 | Archive all state files to `roadmap/archive/` | Carter |
| 2 | Remove file-based write paths | Carter |
| 3 | Disable dual-write (Postgres-only writes) | Carter |
| 4 | Clean up file-based claim locks | Carter |
| 5 | Update CLI to Postgres-only mode | Carter |
...
| Metric | Target | Measurement |
|--------|--------|-------------|
| Postgres-only writes | 100% | Write routing logs |
...
|------|-------------|--------|------------|
| Postgres data corruption | Low | Critical | Dual-write, checksums, instant rollback |
