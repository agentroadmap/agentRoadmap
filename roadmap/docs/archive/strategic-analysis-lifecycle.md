...
* **Transactional Safety:** With the transition to SpacetimeDB, these leases are managed via atomic transactions, preventing race conditions during claim and release.
...
* **Ephemeral Workspaces:** Agents spin up inside a container, pull the required context via the SpacetimeDB/Daemon API, execute their tasks, and the container is destroyed or recycled.
...
* **Evolution:** Build on top of these profiles by adding dynamic, roadmap-specific metadata (e.g., current workload, historical success rate on specific labels, SpacetimeDB connection status).
