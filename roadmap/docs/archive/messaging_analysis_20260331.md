...
| **Protocol** | Uses **ACP (Agent Communication Protocol)** for tool-calling. | Uses **Postgres** for permanent project state. |
...
2.  **Use Postgres for the "Record":** Your messaging module should "listen" to the OpenClaw WebSocket. Every time a message is sent, your module saves it to Postgres.
...
* **Suggestion:** Have your **agentRoadmap** platform automatically generate the `TOOLS.md` file based on the MCP servers you have active. When a new agent spawns in OpenClaw, it reads that file and immediately knows how to talk to your Postgres or your Filesystem.
...
| **Streaming** real-time text chunks. | **Recording** those chunks into Postgres. |
