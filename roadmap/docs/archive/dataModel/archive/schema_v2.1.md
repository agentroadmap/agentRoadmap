...
### **Execution Strategy**
1.  **Nuke the DB:** Recreate the schema from the checked-in migrations.
2.  **Rust Migration:** Implement the above tables as `#[postgres(table)]` structs in your `infrastructure/src/lib.rs`.
...
**Shall we move to the next step—defining the core "Proposal Lifecycle" workflow action in Rust to manage these status transitions?**
