# Field Mapping: .md Frontmatter → SpacetimeDB Tables
**STATE-095 AC#3** | Created: 2026-03-25 15:05 UTC | Author: Carter

## Purpose

Map every field from state `.md` frontmatter and body sections to corresponding SpacetimeDB table columns, ensuring zero data loss during migration.
...
| .md Frontmatter Field | SDB Column | SDB Type | Notes |
|----------------------|------------|----------|-------|
...
| .md Body Section | SDB Column | SDB Type | Notes |
|-----------------|------------|----------|-------|
...
| .md Frontmatter | SDB Column | SDB Type |
|----------------|------------|----------|
| `labels[]` | `label` | `string` |
...
| Parsed From | SDB Column | SDB Type |
|-------------|------------|----------|
...
```typescript
// Frontmatter (ISO string) → SDB (epoch ms)
function frontmatterDateToEpoch(iso: string | undefined): number {
  if (!iso) return Date.now();
  return new Date(iso).getTime();
}

// SDB (epoch ms) → Frontmatter (ISO string)
...
```typescript
// Frontmatter array → SDB comma-separated string
function arrayToSdbField(arr: string[] | undefined): string {
  return (arr ?? []).join(', ');
}

// SDB comma-separated → Frontmatter array
...
Frontmatter uses kebab-case, SDB uses exact enum match:
...
**Coverage: 68% of fields migrate to SDB, 32% are computed/file-only.**
...
- [ ] Implement transform function in `src/spacetimedb/migration-transform.ts`
