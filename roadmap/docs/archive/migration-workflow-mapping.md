...
| Workflow | Current (File) | Target (SpacetimeDB) |
|----------|----------------|---------------------|
| Agent Claiming | Lock files + heartbeat | `claims` table with TTL |
| State Transitions | Frontmatter edit + git | Reducers with validation |
| Merge Process | Git merge + conflict resolution | SDB merge + conflict table |
...
### Target: SpacetimeDB `claims` Table
...
// heartbeat reducer (called periodically)
...
// cleanup_expired_claims reducer (called by scheduler)
...
| Stale lock detection (cron) | `cleanupExpiredClaims()` reducer |

---

## 2. State Transitions → Reducers with Validation
...
### Target: Transition Reducers

```typescript
// transition_state reducer
transitionState(
  stateId: string,
...
| validateStateTransition() | Built into reducer |

---

## 3. Merge Process → SDB Merge Reducers
...
### Target: SpacetimeDB Merge Process

```typescript
// merge_states reducer
mergeStates(
...
// resolve_conflict reducer (manual resolution)
resolveConflict(
  stateId: string,
...
const readySub = spacetimedb.subscribe({
  table: 'states',
  filter: {
    status: 'contracted',
...
|------|--------|----------|
| 1 | Deploy `claims` table + reducers | None |
| 2 | Dual-write claims (file + SDB) | None |
| 3 | Switch claim reads to SDB | None |
| 4 | Deploy transition reducers | None |
| 5 | Dual-write transitions | None |
| 6 | Deploy merge reducers | None |
...
- [ ] Implement merge reducers in state-storage.ts
