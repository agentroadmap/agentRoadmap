...
The migration strategy proposes SpacetimeDB with reducers as the access boundary. But the **interim** phase (SQLite as local cache, dual-write) has a weaker security posture than the current file model:

| Concern | File Model | SQLite Interim | SpacetimeDB Target |
|---|---|---|---|
| **Read access** | Any process | Any process with DB path | Authenticated client |
| **Write access** | Any process with UID | Any process with DB path | Reducer-validated |
| **Row-level control** | None (file per state) | None (single table) | Reducer logic |
...
| **Phase 3** (Read flip) | SpacetimeDB | SQLite cache stale | TTL + invalidation |
| **Phase 4** (Cutover) | SpacetimeDB | .md projection lags | Projection checksum |
...
|---|---|---|
| SpacetimeDB failure > 10% | Auto-disable writes | **Also:** Freeze agent_tokens, reject new auth |
...
**Problem:** Rolling back from Phase 3 (SpacetimeDB reads) to Phase 1 (file reads) requires ensuring files are current.

**Current rollback procedure** (migration strategy §3.2):
```
1. Disable SpacetimeDB writes
