...
const reviews = [
  ...
  { pid: 184, notes: "Design field empty - no implementation specifics, no file/function targets, no role-tag schema" },
  { pid: 185, notes: "Design field empty - no schema definition, no data model, no INSERT/query logic specified" },
  { pid: 199, notes: "Replay attack not in threat model; architecture option A/B/C not selected; implementer cannot proceed" },
];
...
for (const r of reviews) {
  ...
  await query(
    `INSERT INTO roadmap_proposal.proposal_reviews (proposal_id, reviewer_identity, verdict, is_blocking, notes, findings)
     VALUES ($1, 'system', 'FAIL', true, $2, $3::jsonb)
