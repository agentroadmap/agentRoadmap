...
Archive spending-control notes still support these rules:

1. **Visibility must exist at multiple levels.** Track spend by agent, proposal, team, model, and overall system.
2. **Budget controls need both telemetry and intervention.** Dashboards are not enough without freeze, throttle, or escalation paths.
3. **Cache-aware accounting matters.** Cache hits and reused context should be visible so optimization work can be measured.
4. **Human override remains important.** The system can recommend throttling, but human operators still need an explicit circuit-breaker path for exceptional cases.
5. **Financial state should be durable.** Token usage, spend events, and freeze actions belong in queryable ledgers, not transient interface state.
