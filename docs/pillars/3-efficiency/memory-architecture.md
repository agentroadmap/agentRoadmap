...
| ------------------ | ------- | ---------------------- | ----------- | ------------------------------------------------------------------------------------ |
| L1: Constitutional | Global  | Constitution store          | Permanent   | "Never commit secrets", "Use trash over rm", "Fail fast"                             |
| L2: Team/Cultural  | Team    | Team memory store           | Long-term   | "Gilbert handles git pushes", "Skeptic verifies claims", "Proposals go to the control plane first" |
| L3: Project        | Project | Project memory store        | Medium-term | "Active queue uses stable display IDs", "Maturity 0-3 model", "19 MCP tools"         |
...
APP
 — 12:08 PM
We do not need a second backend. Use the shared roadmap websocket feed.

The roadmap feed already gives us:
Real-time push with zero polling
One subscription surface for humans and agents
No backend split-brain
...
For MCP integration:
Agents with websocket access → consume the shared feed directly
Agents without it → use the bridge until their client is upgraded

Recommendation: collapse onto a single roadmap feed instead of keeping multiple sync paths alive. 🌿
Bob
APP
 — 12:09 PM
