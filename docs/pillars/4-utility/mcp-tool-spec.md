...
1. **One record family → one domain.** Tools map directly to the control-plane entities they manage.
...
The archive surfaced a few MCP design principles that are still valid after the Postgres cutover:

1. **Tool exposure should stay scoped.** Agents do better when they see the smallest useful tool surface for the task at hand.
2. **Durable record and live transport are different concerns.** Messaging and event delivery can be real-time, but important actions still need a queryable durable record.
3. **Descriptions matter as much as names.** Tool discoverability depends heavily on clear, domain-accurate descriptions.
4. **Filesystem-era message logs are no longer canonical.** MCP tools should read/write through the current control-plane model rather than assuming markdown logs are truth.
...
| `agent_pulse` | Update agent status freshness | `identity`, `active_proposal_id?`, `status_message?` | `workforce_pulse` |
| `agent_heartbeat` | Quick freshness signal (minimal update) | `identity` | `workforce_pulse` |
| `agent_report` | Get agent status report (pulse + current task) | `identity` | — (read, `workforce_pulse`) |

Telemetry safety contract: `agent_pulse` and `agent_heartbeat` are advisory freshness signals only. Missing updates may mark an agent as needing attention after an extended silence window, but they must never auto-freeze spend, revoke access, or declare the agent dead without explicit human approval.
...
| Tool | Description | Key Inputs | Writes to |
|------|-------------|-----------|-----------|
| `sync_run` | Trigger a sync from the control plane to the filesystem | `proposal_id?` (all if omitted) | Filesystem |
...
Each tool calls a control-plane workflow action. Required workflow actions:

| workflow action | Called by |
|---------|-----------|
...
| `record_agent_status` | `agent_pulse` |
...
Bob's module has 4 workflow actions currently. We'll need ~15 total to cover all tools.
