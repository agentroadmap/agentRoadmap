...
# Design Document: `pipeline/` Domain (The Gatekeeper)

## 1. The "Promotion" Workflow
Agents do not commit to `main`. They "promote" artifacts through a three-stage environment managed in Postgres.
...
## 3. Automated Code Review (The "Sonar" Layer)
...
3.  **The Human Approval Gate:** Even if all tests pass, the final move to `main` requires a **one-click approval** from your **Mobile App** or **WebSash**.

---

## 5. Organizing the `pipeline/` Directory
...
## Carried-forward verification rules

The archive review reduced earlier testing and review notes into a few durable rules:

1. **Proof beats assertion.** If a compiler, test runner, linter, or deterministic check can verify something, prefer that over AI judgment.
2. **Evidence should be structured.** Proposal promotion should carry explicit proof such as test output, acceptance-criteria status, screenshots, or review rationale.
3. **Hard-fail categories stay explicit.** Secret exposure, broken builds, missing required acceptance criteria, and schema-invalid transitions should stop promotion immediately.
4. **Peer review is a gate, not decoration.** Reviews should capture why work is acceptable, not just that someone glanced at it.
5. **Pre-flight checks should happen early.** Fast local verification is cheaper than discovering basic breakage deep in a promotion flow.
