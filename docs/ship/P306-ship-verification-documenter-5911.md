# P306 Ship Verification — worker-5911 (documenter)

Date: 2026-04-21 07:24 UTC
...
|----|-------------|--------|
| AC-1 | All proposal.status values UPPERCASE | PASS — 6 canonical values: COMPLETE(72), DRAFT(36), DEPLOYED(34), DEVELOP(31), REVIEW(10), MERGE(1) |
| AC-2 | Migration SQL verified | PASS — 044-normalize-proposal-status-casing.sql applied, data normalized |
| AC-3 | LOWER() removed from status comparisons | PASS — grep confirms 0 LOWER(status) in scripts/orchestrator.ts and scripts/bootstrap-state-machine.ts |
| AC-4 | CHECK constraint prevents mixed-case | PASS — proposal_status_canonical constraint active on roadmap_proposal.proposal (28 reference_terms values) |
...
- src/core/pipeline/pipeline-cron.ts:1278: LOWER() preserved (intentional, cross-table comparison against transition_queue.to_stage title-case) ✓ (1 match)
- src/infra/postgres/proposal-storage-v2.ts:342: toUpperCase() input guard in createProposal() ✓ (2 matches)
- Trigger: trg_normalize_proposal_status — active (BEFORE INSERT, BEFORE UPDATE) ✓
- CHECK: proposal_status_canonical — active (28 values) ✓
- normalizeState() preserved as defense-in-depth in orchestrator.ts ✓

## History

Prior verifications: worker-5859, worker-5668, worker-5624, worker-5815, worker-5763, worker-5762. All 8/8 PASS.
Counts shifted slightly from prior (DEVELOP 30→31, REVIEW 11→10) — normal proposal lifecycle, expected behavior.
